  ...
  const rule: RuleFunction = async (context: RuleContext): Promise<void> => {
    ...
    for (const layer of utils.objects.anyLayer) {
      ...
      if (!library) {
        ...
        utils.report(i18n._(t`A shared style from a library is expected`), [layer])
        continue
      }
      ...
      if (Array.isArray(authorizedLibraries) && authorizedLibraries.length > 0) {
        ...
        if (!isAuthorized) {
          utils.report(i18n._(t`Uses the unauthorized library "${libraryName}"`), [layer])
          continue
        }
      }
      ...
      if (!isStyleEq) {
        utils.report(i18n._(t`Shared style differs from library`), [layer])
      }
    }
  }
