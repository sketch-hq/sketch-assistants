...
describe('text-styles-no-dirty', () => {
  ...
  test('finds violations for text styles that differ from their shared styles', async (): Promise<void> => {
    expect.assertions(2)
    const { violations, ruleErrors } = await testCoreRule(
      __dirname,
      './no_good.sketch',
  ...
  test('outputs no violations for groups that differ from their shared style', async (): Promise<void> => {
    expect.assertions(2)
    const { violations, ruleErrors } = await testCoreRule(
      __dirname,
      './groups_bad.sketch',
